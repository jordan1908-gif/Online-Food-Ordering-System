...
<?php
    ...
    $sql = "SELECT username,email,contact,address,gender,position,id from staff WHERE id=$id";
    ...
    if ($result-> num_rows>0) {
        while ($row = $result -> fetch_assoc()) {
            ...
            echo 'Position:<br>'.$row["position"].'<br><br>';
            echo 'Email:<br>'.$row["email"].'<br><br>';
            echo 'Contact:<br>'.$row["contact"].'<br><br>';
            echo 'Address:<br>'.$row["address"].'<br><br>';
            echo '<a class="btn btn-success" href="edit2.php?id='.$row['id'].'">Update Details</a>';
            echo '</div>';
